...
const TechHeader = () => {
   ...
   return (
      ...
      <div className="mb-4">
         <h1 className="font-bold leading-tight mb-4" style={{ fontSize: globalFontSize.name, color: theme.color }}>
            {personalInfo.fullName || (
               <span className="text-gray-400 italic font-normal">{DUMMY_DATA.personalInfo.fullName}</span>
            )}
         </h1>
         <p className="text-gray-500 mb-2" style={{ fontSize: globalFontSize.tagline }}>
            {personalInfo.tagline.length > 0 ? (
               personalInfo.tagline.join(' | ')
            ) : (
               <span className="text-gray-400 italic">{DUMMY_DATA.personalInfo.tagline}</span>
            )}
         </p>
         <p className="text-gray-500 mb-2" style={{ fontSize: globalFontSize.summary }}>
